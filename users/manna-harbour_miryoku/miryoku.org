...
#+BEGIN_SRC python :var alphas_table=colemakdh :var thumbs_table=thumbs :var hold_table=hold :var half_hold="LR" :var symbol_names_table=symbol-names :var mods_table=mods :var nonkc_table=nonkc :tangle no :results verbatim
width = 19
...
for tap_row, hold_row in zip(alphas_table + thumbs_table, hold_table):
  ...
  for idx, (tap, hold) in enumerate(zip(tap_row, hold_row)):
    ...
    if not str(code).startswith(nonkc_tuple):
      code = 'KC_' + str(code)
    if 'L' in half_hold and idx < 5 or 'R' in half_hold and idx >= 5:
...
#define MIRYOKU_TABLE_COLEMAK_NO_HOLD_FULL \
  <<table-layout-taphold(alphas_table=colemak, half_hold="")>>
#define MIRYOKU_TABLE_COLEMAKDHK_NO_HOLD_FULL \
  <<table-layout-taphold(alphas_table=colemakdhk, half_hold="")>>
#define MIRYOKU_TABLE_DVORAK_NO_HOLD_FULL \
  <<table-layout-taphold(alphas_table=dvorak, half_hold="")>>
#define MIRYOKU_TABLE_HALMAK_NO_HOLD_FULL \
  <<table-layout-taphold(alphas_table=halmak, half_hold="")>>
#define MIRYOKU_TABLE_WORKMAN_NO_HOLD_FULL \
  <<table-layout-taphold(alphas_table=workman, half_hold="")>>
#define MIRYOKU_TABLE_QWERTY_NO_HOLD_FULL \
  <<table-layout-taphold(alphas_table=qwerty, half_hold="")>>
#define MIRYOKU_TABLE_COLEMAKDH_NO_HOLD_FULL \
  <<table-layout-taphold(alphas_table=colemakdh, half_hold="")>>

#define MIRYOKU_TABLE_COLEMAK_NO_HOLD_R \
  <<table-layout-taphold(alphas_table=colemak, half_hold="L")>>
#define MIRYOKU_TABLE_COLEMAKDHK_NO_HOLD_R \
  <<table-layout-taphold(alphas_table=colemakdhk, half_hold="L")>>
#define MIRYOKU_TABLE_DVORAK_NO_HOLD_R \
  <<table-layout-taphold(alphas_table=dvorak, half_hold="L")>>
#define MIRYOKU_TABLE_HALMAK_NO_HOLD_R \
  <<table-layout-taphold(alphas_table=halmak, half_hold="L")>>
#define MIRYOKU_TABLE_WORKMAN_NO_HOLD_R \
  <<table-layout-taphold(alphas_table=workman, half_hold="L")>>
#define MIRYOKU_TABLE_QWERTY_NO_HOLD_R \
  <<table-layout-taphold(alphas_table=qwerty, half_hold="L")>>
#define MIRYOKU_TABLE_COLEMAKDH_NO_HOLD_R \
  <<table-layout-taphold(alphas_table=colemakdh, half_hold="L")>>

#define MIRYOKU_TABLE_COLEMAK_NO_HOLD_L \
  <<table-layout-taphold(alphas_table=colemak, half_hold="R")>>
#define MIRYOKU_TABLE_COLEMAKDHK_NO_HOLD_L \
  <<table-layout-taphold(alphas_table=colemakdhk, half_hold="R")>>
#define MIRYOKU_TABLE_DVORAK_NO_HOLD_L \
  <<table-layout-taphold(alphas_table=dvorak, half_hold="R")>>
#define MIRYOKU_TABLE_HALMAK_NO_HOLD_L \
  <<table-layout-taphold(alphas_table=halmak, half_hold="R")>>
#define MIRYOKU_TABLE_WORKMAN_NO_HOLD_L \
  <<table-layout-taphold(alphas_table=workman, half_hold="R")>>
#define MIRYOKU_TABLE_QWERTY_NO_HOLD_L \
  <<table-layout-taphold(alphas_table=qwerty, half_hold="R")>>
#define MIRYOKU_TABLE_COLEMAKDH_NO_HOLD_L \
  <<table-layout-taphold(alphas_table=colemakdh, half_hold="R")>>
...
#define MIRYOKU_TABLE_COLEMAK_FLIP_NO_HOLD_FULL \
  <<table-layout-taphold(alphas_table=colemak, thumbs_table=thumbs-flip, hold_table=hold-flip, half_hold="")>>
#define MIRYOKU_TABLE_COLEMAKDHK_FLIP_NO_HOLD_FULL \
  <<table-layout-taphold(alphas_table=colemakdhk, thumbs_table=thumbs-flip, hold_table=hold-flip, half_hold="")>>
#define MIRYOKU_TABLE_DVORAK_FLIP_NO_HOLD_FULL \
  <<table-layout-taphold(alphas_table=dvorak, thumbs_table=thumbs-flip, hold_table=hold-flip, half_hold="")>>
#define MIRYOKU_TABLE_HALMAK_FLIP_NO_HOLD_FULL \
  <<table-layout-taphold(alphas_table=halmak, thumbs_table=thumbs-flip, hold_table=hold-flip, half_hold="")>>
#define MIRYOKU_TABLE_WORKMAN_FLIP_NO_HOLD_FULL \
  <<table-layout-taphold(alphas_table=workman, thumbs_table=thumbs-flip, hold_table=hold-flip, half_hold="")>>
#define MIRYOKU_TABLE_QWERTY_FLIP_NO_HOLD_FULL \
  <<table-layout-taphold(alphas_table=qwerty, thumbs_table=thumbs-flip, hold_table=hold-flip, half_hold="")>>
#define MIRYOKU_TABLE_COLEMAKDH_FLIP_NO_HOLD_FULL \
  <<table-layout-taphold(alphas_table=colemakdh, thumbs_table=thumbs-flip, hold_table=hold-flip, half_hold="")>>

#define MIRYOKU_TABLE_COLEMAK_FLIP_NO_HOLD_R \
  <<table-layout-taphold(alphas_table=colemak, thumbs_table=thumbs-flip, hold_table=hold-flip, half_hold="L")>>
#define MIRYOKU_TABLE_COLEMAKDHK_FLIP_NO_HOLD_R \
  <<table-layout-taphold(alphas_table=colemakdhk, thumbs_table=thumbs-flip, hold_table=hold-flip, half_hold="L")>>
#define MIRYOKU_TABLE_DVORAK_FLIP_NO_HOLD_R \
  <<table-layout-taphold(alphas_table=dvorak, thumbs_table=thumbs-flip, hold_table=hold-flip, half_hold="L")>>
#define MIRYOKU_TABLE_HALMAK_FLIP_NO_HOLD_R \
  <<table-layout-taphold(alphas_table=halmak, thumbs_table=thumbs-flip, hold_table=hold-flip, half_hold="L")>>
#define MIRYOKU_TABLE_WORKMAN_FLIP_NO_HOLD_R \
  <<table-layout-taphold(alphas_table=workman, thumbs_table=thumbs-flip, hold_table=hold-flip, half_hold="L")>>
#define MIRYOKU_TABLE_QWERTY_FLIP_NO_HOLD_R \
  <<table-layout-taphold(alphas_table=qwerty, thumbs_table=thumbs-flip, hold_table=hold-flip, half_hold="L")>>
#define MIRYOKU_TABLE_COLEMAKDH_FLIP_NO_HOLD_R \
  <<table-layout-taphold(alphas_table=colemakdh, thumbs_table=thumbs-flip, hold_table=hold-flip, half_hold="L")>>

#define MIRYOKU_TABLE_COLEMAK_FLIP_NO_HOLD_L \
  <<table-layout-taphold(alphas_table=colemak, thumbs_table=thumbs-flip, hold_table=hold-flip, half_hold="R")>>
#define MIRYOKU_TABLE_COLEMAKDHK_FLIP_NO_HOLD_L \
  <<table-layout-taphold(alphas_table=colemakdhk, thumbs_table=thumbs-flip, hold_table=hold-flip, half_hold="R")>>
#define MIRYOKU_TABLE_DVORAK_FLIP_NO_HOLD_L \
  <<table-layout-taphold(alphas_table=dvorak, thumbs_table=thumbs-flip, hold_table=hold-flip, half_hold="R")>>
#define MIRYOKU_TABLE_HALMAK_FLIP_NO_HOLD_L \
  <<table-layout-taphold(alphas_table=halmak, thumbs_table=thumbs-flip, hold_table=hold-flip, half_hold="R")>>
#define MIRYOKU_TABLE_WORKMAN_FLIP_NO_HOLD_L \
  <<table-layout-taphold(alphas_table=workman, thumbs_table=thumbs-flip, hold_table=hold-flip, half_hold="R")>>
#define MIRYOKU_TABLE_QWERTY_FLIP_NO_HOLD_L \
  <<table-layout-taphold(alphas_table=qwerty, thumbs_table=thumbs-flip, hold_table=hold-flip, half_hold="R")>>
#define MIRYOKU_TABLE_COLEMAKDH_FLIP_NO_HOLD_L \
  <<table-layout-taphold(alphas_table=colemakdh, thumbs_table=thumbs-flip, hold_table=hold-flip, half_hold="R")>>
